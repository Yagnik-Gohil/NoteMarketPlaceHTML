...
namespace NotesMarketPlace.Controllers
{
    public class AllMembersController : Controller
    {
        ...
        public ActionResult AllMembers(string search, int? page, string sortby)
        {
            ...
            System.Linq.IQueryable<NotesMarketPlace.Context.UserTable> filtered;     //Empty Variable for Holding Members
            ...
            else
            {
                ...
                filtered = dbobj.UserTable.Where(x => x.IsActive && (x.FirstName.Contains(search) || x.LastName.Contains(search) ||
                x.Email.Contains(search) || (x.CreatedDate.Value.Day + "-" + x.CreatedDate.Value.Month + "-" + x.CreatedDate.Value.Year).Contains(search))).ToList().AsQueryable();
            }
            ...
            foreach (var item in entry)
            {
                mobj.Add(new Members()
                {
                    ...
                    PublishedNotes = dbobj.NoteTable.Where(x => x.UID == item.UID && x.Status == 4).Count(),
                    DownloadedNotes = dbobj.TransectionTable.Where(x => x.BuyerID == item.UID && x.IsDownloaded == true).Count(),
                    TotalExpenses = dbobj.TransectionTable.Where(x => x.BuyerID == item.UID && x.IsDownloaded == true).Count() == 0 ? 0 : dbobj.TransectionTable.Where(x => x.BuyerID == item.UID && x.IsDownloaded == true).Sum(x => x.Price),
                    TotalEarnings = dbobj.TransectionTable.Where(x => x.SellerID == item.UID && x.IsAllowed == true).Count() == 0 ? 0 : dbobj.TransectionTable.Where(x => x.SellerID == item.UID && x.IsAllowed == true).Sum(x => x.Price),
                    IsActive = item.IsActive
                });
        ...
        public ActionResult Deactivate(int uid)
        {
            ...
            Context.UserTable userobj = dbobj.UserTable.Where(x => x.UID == uid).FirstOrDefault();
            UserProfileTable upobj = dbobj.UserProfileTable.Where(x => x.UID == uid).FirstOrDefault();

            userobj.IsActive = false;
            upobj.IsActive = false;

            var usernotes = dbobj.NoteTable.Where(x => x.UID == uid);
